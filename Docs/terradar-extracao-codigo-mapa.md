# TERRADAR - Extração de Código - Aba Mapa

Consolidação de ficheiros para análise externa. Caminhos relativos à raiz do projeto.
...
## 2. Definições de Substâncias
**Arquivo:** `src/lib/substancias.ts` (`SUBSTANCIA_DEFS`, ícones, cores, helpers).
```ts
import {
  Atom,
  CircleDot,
...
**Outros candidatos (não presentes neste repositório):** `src/lib/substanciasDestaque.ts`, `src/lib/substanciasAgrupadas.ts`, `src/components/map/MapSubstanciasFiltroDropdown.tsx`

## 3. Store do Mapa (Zustand)
...
## 4. Componente da Legenda
**ARQUIVO DEDICADO NÃO ENCONTRADO:** `MapLegend.tsx`, `Legend.tsx` (procurados em `src/`).

A UI da legenda (regimes e camadas geo ativas no canto inferior direito; painel Risk Score no canto superior direito quando o modo de visualização é risco) está no mesmo ficheiro que a vista do mapa. Ver secção 6 (`MapView.tsx`).

## 5. Barra de Busca do Mapa
...
## 6. Camada de Polígonos / Mapbox / Legenda (integrada)
**Arquivo:** `src/components/map/MapView.tsx` (inclui `buildGeoJSON`, `addProcessosLayers`, `fill-color` via propriedade `color`, legenda de regimes e Risk Score)
```tsx
