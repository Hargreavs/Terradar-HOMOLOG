  ...
  const infraRows = data.infraestrutura
    .map((i) => {
      ...
      const nomeCell = isRail
        ? `<strong>${sanitizeReportText(i.nome)}</strong>${emEstudo ? ' <span style="font-size:7pt;color:#666;">(sem operação)</span>' : ''}`
        : `${sanitizeReportText(i.nome)}${emEstudo ? ' <span style="font-size:7pt;color:#666;">(sem operação)</span>' : ''}`
      return `<tr>
  ...
  <div class="cols">
    ...
    <div class="col">
      ...
      <table class="dsm">
        ...
        <tr><td>${sanitizeReportText(t.cfemAliquotaLabel)}</td><td>${fmtPct(data.cfem_aliquota_pct)} ${sanitizeReportText(t.tblCfemPct)}</td></tr>
