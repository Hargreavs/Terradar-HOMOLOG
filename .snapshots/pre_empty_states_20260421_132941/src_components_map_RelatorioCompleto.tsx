...
function montarExibicaoScoresRelatorio(
  ...
  rsCorNumero: string
  rsCorRotulo: string
  osLabel: (valorPerfil: number) => string
  osCor: (valorPerfil: number) => string
} {
  const se = scoresApi
  ...
  return {
    ...
    rsCorNumero,
    rsCorRotulo,
    osLabel: (valorPerfil: number) =>
      osPersist && osPersist !== ''
        ? osPersist
        : getOpportunityLabel(valorPerfil),
    osCor: (valorPerfil: number) =>
      osCorPersist && osCorPersist !== ''
        ? osCorPersist
        : corFaixaOS(valorPerfil),
  }
}
  ...
  const exibicaoScoresRelatorio = useMemo(() => {
    if (!processo) {
      return {
        ...
        rsCorNumero: '#888780',
        rsCorRotulo: '#888780',
        osLabel: (valorPerfil: number) => getOpportunityLabel(valorPerfil),
        osCor: (valorPerfil: number) => corFaixaOS(valorPerfil),
      }
    }
          ...
          <>
            {semGeom ? (
              ...
              >
                ...
                georreferenciado no SIGMINE. Dados cadastrais, regulatórios e
                fiscais estão disponíveis. A análise territorial não pode ser
                gerada.
              </div>
            ) : null}
            <Card>
