...
async function fetchZipResponse(): Promise<{ res: Response; urlUsed: string }> {
  ...
  for (let i = 0; i < BACKOFF_MS.length; i++) {
    if (BACKOFF_MS[i]! > 0) await sleep(BACKOFF_MS[i]!)
    try {
      const res = await fetch(URL_PRIM, { signal: AbortSignal.timeout(7200_000) })
  ...
  if (only5xx && lastSt >= 500) {
    ...
    for (let i = 0; i < BACKOFF_MS.length; i++) {
      if (BACKOFF_MS[i]! > 0) await sleep(BACKOFF_MS[i]!)
      try {
        const res = await fetch(URL_ESP, { signal: AbortSignal.timeout(7200_000) })
  ...
  throw new Error(`download SIGMINE falhou (ultimo HTTP ${lastSt})`)
}

async function headSig(u: string): Promise<{ et: string | null; lm: string | null }> {
  try {
    const r = await fetchBackoff(() => fetch(u, { method: 'HEAD', signal: AbortSignal.timeout(60_000) }), 'HEAD')
