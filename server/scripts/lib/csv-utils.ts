...
/**
 * Stream transform que remove aspas internas não escapadas nos campos de CSV.
 * Problema: CSVs da ANM às vezes contêm aspas literais dentro de campos (ex: VEND"AGUA),
 * sem escape duplo (""). O csv-parse aborta nesses casos.
 *
 * Solução: remover aspas que estejam cercadas por caracteres que NÃO são
 * separador (,;) nem quebra de linha (\r\n).
 *
 * Ex: `"VEND"AGUA LTDA"` → `"VENDAGUA LTDA"` (aspa interna removida)
 *     `"FOO","BAR"` → preservado (aspas em fronteira de campo)
 *
 * NOTA: opera em chunks de string. Para robustez entre chunks, mantém buffer de 1 char.
 */
export function criarStreamLimpezaAspas(): Transform {
  let buffer = ''

  const regexAspaInterna = /(?<=[^;,\r\n])"(?=[^;,\r\n])/g

  return new Transform({
    transform(chunk: Buffer | string, _encoding, callback) {
      const text = typeof chunk === 'string' ? chunk : chunk.toString('utf8')
      const combinado = buffer + text
      if (combinado.length > 0) {
        buffer = combinado.slice(-1)
        const processar = combinado.slice(0, -1)
        callback(null, processar.replace(regexAspaInterna, ''))
      } else {
        callback(null, '')
      }
    },
    flush(callback) {
      callback(null, buffer.replace(regexAspaInterna, ''))
      buffer = ''
    },
  })
}
