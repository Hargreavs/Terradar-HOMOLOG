...
function getProfile(elapsed: number): ProfileState {
  ...
  for (let i = 0; i < PROFILES.length; i++) {
    if (t < acc + PROFILES[i].duration) {
      ...
      return {
        ...
        dominantColor:
          blend < 0.5
            ? PROFILES[i].weights[0] > 35
              ? COLORS.verde
              : PROFILES[i].weights[2] > 40
                ? COLORS.ambar
                : COLORS.ambarRisco
            : next.weights[0] > 35
              ? COLORS.verde
  ...
  return (
    ...
    >
      ...
      >
          ...
          return [0, 1].map((w) => {
            ...
            return (
              <circle
                ...
                strokeWidth={thickness * (1 - phase * 0.5)}
                filter={weight > 0.35 ? 'url(#rgq)' : undefined}
              />
            )
          })
        })}
        ...
        {MINERALS.map((m, i) => {
          ...
          DIMENSIONS.forEach((d, di) => {
            const dd = Math.abs(dist - d.baseRadius)
            if (dd < cdist) {
              cdist = dd
              cd = di
            }
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div
            style={{
              ...
              height: 7,
              borderRadius: '50%',
              background: profile.dominantColor,
              boxShadow: `0 0 10px ${profile.dominantColor}`,
      ...
      <div style={{ position: 'absolute', bottom: 20, left: 24, right: 24 }}>
        ...
        >
          Pesos da Pontuação de Oportunidade
        </div>
        {DIMENSIONS.map((dim, di) => {
