/** Durações e helpers legados do motion system (App + Inteligência + Mapa). */
/** Para código novo, preferir tokens semânticos em `./motion.ts` (re-export ao final). */

/**
 * Mantido para retrocompatibilidade — preferir tokens semânticos de motion.ts em código novo.
 * Overlay inner e sequência de entrada.
 */
...
/** Recharts `<Line>` animationBegin por `dataKey` (SUB_KEYS). Mantido específico do Recharts. */
